...
The example we will use in this section comes from @Strijbosch1988, who quantified the age-specific demography of the viviparous lizard (*Zootoca vivipara*) in an intensive 7-year study on a 1.2 ha plot at the forest reserve "de Hamert" in the Netherlands.
The age-specific female birth and survival rates are shown in the following table:
...
lizard <- data.frame(x = 0:7, px = c(round(px, 3), 0), bx = c(0, bx))
knitr::kable(
  lizard,
  ...
  col.names = c("Age in years ($x$)", "Female annual survival ($P_x$)", "Female offspring per female ($m_x$)") 
)
```
